...
class FlowRoutingView(HTTPMethodView):
    ...
    def post(self, request):
        """post method for flow/routing api update device config? into database how they use it --> policy_monitor_task.py"""
        try:
            actions = []
        ...
        obj_id = str(policy['new_flow']['src_ip']) + "-" + str(policy['new_flow']['src_port']) + "-" +  str(policy['new_flow']['dst_ip']) + "-" +  str(policy['new_flow']['dst_port'])
